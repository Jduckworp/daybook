...
# BASE locates the templates and static files that ship inside the package.
# The database and config do NOT live there — an installed package sits in
# site-packages, which must stay read-only — so they default to the working
# directory instead. For a git checkout run from its own root, and for the
# systemd unit whose WorkingDirectory is the install root, that is the same
# ./data and ./config.json as before. Docker sets both paths explicitly.
BASE = Path(__file__).resolve().parent
RUNDIR = Path.cwd()
DATA = Path(os.environ.get("DONEBOOK_DATA_DIR") or RUNDIR / "data")
